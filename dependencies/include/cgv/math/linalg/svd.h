...
T pythag(const T a, const T b)
{
	...
	absa=fabs(a);
	absb=fabs(b);
	if (absa > absb) return (T)absa*sqrt(1+SQR(absb/absa));
	else return (T)(absb == 0.0 ? 0.0 : absb*sqrt(1.0+SQR(absa/absb)));
}
...
template <class T>
void svd( cgv::math::linalg::mat<T> &a, cgv::math::linalg::diag_mat<T> &w,  cgv::math::linalg::mat<T> &v)
{
	bool flag;
	...
	cgv::math::linalg::col_vec<T> rv1(n);
	g=scale=anorm=0.0;
	for (i=0;i<n;i++) {
		l=i+2;
		rv1(i)=scale*g;
