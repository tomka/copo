...
ds_audio_device_iterator::ds_audio_device_iterator()
{
	ensure_com();
	...
	if (FAILED(hr = pDevEnum->CreateClassEnumerator (CLSID_AudioInputDeviceCategory, &pClassEnum, 0))) {
...
void ds_audio_device_iterator::operator ++()
{
	if (pMoniker != NULL)
		pMoniker->Release();
	if (pClassEnum == NULL) {
...
bool ds_audio_device_iterator::at_end() const
{
	return pMoniker == NULL;
}
IMoniker* ds_audio_device_iterator::operator -> ()
{
	return pMoniker;
}
std::string ds_audio_device_iterator::get_name() 
{
	if (pMoniker == NULL)
		return std::string();
