...
template class delaunay_mesh_with_hierarchy<delaunay_mesh<triangle_mesh<mesh_geometry<float> > > >;
	...
	const point_type& p, unsigned int) const
{
	...
	return T::triangle_mesh_type::localize_point(p, this->ci_of_vi(vi));
}
...
unsigned int delaunay_mesh_with_hierarchy<T>::find_nearest_neighbor(const point_type& p, unsigned int) const
{
	...
	for (unsigned int hi = 0; hi < H.size(); ++hi)
		vi = H[hi]->find_nearest_neighbor(p, H[hi]->ci_of_vi(vi));
	return delaunay_mesh_type::find_nearest_neighbor(p, this->ci_of_vi(vi));
}
...
template <class T>
typename delaunay_mesh_with_hierarchy<T>::vertex_insertion_info delaunay_mesh_with_hierarchy<T>::insert_vertex(unsigned int vi, unsigned int ci_start)
{
	vertex_insertion_info vii = delaunay_mesh_type::insert_vertex(vi,ci_start);
	if (vii.insert_error)
		return vii;
