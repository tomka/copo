...
template class triangle_mesh<mesh_geometry<float> >;
template class triangle_mesh<mesh_geometry<double> >;
template class triangle_mesh<mesh_geometry_reference<float> >;
...
{
	if (geometry_type::is_outside(this->p_of_vi(v0),this->p_of_vi(v1),this->p_of_vi(v2)))
...
{
	unsigned int c1 = this->next(c0);
	unsigned int c2 = this->next(c1);
	return geometry_type::is_outside(p, this->p_of_vi(this->vi_of_ci(c1)), this->p_of_vi(this->vi_of_ci(c2)));
}
...
{
	unsigned int c1 = this->prev(c0);
	unsigned int c2 = this->prev(c1);
	return geometry_type::is_outside(p, this->p_of_vi(this->vi_of_ci(c1)), this->p_of_vi(this->vi_of_ci(c2)));
}
	...
	triangle_mesh<G,C>::localize_point(const point_type& p, unsigned int start_ci) const
{
	...
	while (true) {
		...
		unsigned int i;
		for (i=0; i<k; ++i) {
			bool is_border = this->is_opposite_to_border(ci);
			if (is_outside(p, ci)) {
				if (is_border) {
					pli.is_outside = true;
					pli.ci = ci;
...
//	std::cout << "build_convex_fan_on_border(" << ci << "," << vi << ")" << std::endl;
	this->build_triangle_on_border_edge(ci,vi);
	unsigned int cj = ci;
	while (true) {
		cj = this->next(this->inv(cj));
		unsigned int cb = this->next_on_border(cj);
		unsigned int vb = this->vi_of_ci(this->next(cb));
//		std::cout << "   check vertex " << vb << " to edge (" << vi_of_ci(next(cj)) << "," << vi_of_ci(prev(cj)) << ")" << std::endl;
		if (!this->is_outside(this->p_of_vi(vb), cj))
			break;
		this->build_triangle_connection_to_next_border_edge(cj);
		cj = cb;
	}
	cj = ci;
	while (true) {
		cj = this->prev(this->inv(cj));
		unsigned int cb = this->prev_on_border(cj);
		unsigned int vb = this->vi_of_ci(this->prev(cb));
//		std::cout << "   check vertex " << vb << " to edge (" << vi_of_ci(next(cj)) << "," << vi_of_ci(prev(cj)) << ")" << std::endl;
		if (!this->is_outside(this->p_of_vi(vb), cj))
			break;
		this->build_triangle_connection_to_prev_border_edge(cj);
		cj = cb;
	}
}
...
typename triangle_mesh<G,C>::vertex_insertion_info triangle_mesh<G,C>::insert_vertex(unsigned int vi, unsigned int start_ci)
{
	...
	if (pli.is_outside) {
		...
		const point_type& p  = this->p_of_vi(vi);
		const point_type& p0 = this->p_of_vi(this->vi_of_ci(pli.ci));
		const point_type& p1 = this->p_of_vi(this->vi_of_ci(this->next(pli.ci)));
		const point_type& p2 = this->p_of_vi(this->vi_of_ci(this->prev(pli.ci)));
		coord_type eps = epsilon<coord_type>::get_eps()*std::min(std::min(this->sqr_dist(p0,p1),this->sqr_dist(p1,p2)),this->sqr_dist(p0,p2));
		if (this->sqr_dist(p, p0) <eps) {
...
{
	if (this->get_nr_vertices() < 3)
		return;
	add_triangle(0,1,2);
	for (unsigned int vi=3; vi<this->get_nr_vertices(); ++vi)
		insert_vertex(vi);
}
...
{
	unsigned int ci = this->ci_of_vi(vi);
	unsigned int ck = ci;
	const point_type& pi = this->p_of_vi(vi);
	const point_type& pj = this->p_of_vi(vj);
	do {
		bool trace_strip = true;
		// trace from vertex

		// first check if the target vertex is adjacent to current vertex
		neighbor_cycler nc = this->get_nbr_cycler(ck);
		for (; !nc.cycle_complete(); nc.next()) {
			if (vj == this->vi_of_ci(nc.ci_of_nbr())) {
		...
		const point_type& pk = this->p_of_vi(this->vi_of_ci(ck));
		for (nc = this->get_nbr_cycler(ck); !nc.cycle_complete(); nc.next()) {
			unsigned int      vn = this->vi_of_ci(nc.ci_of_nbr());
			const point_type& pn = this->p_of_vi(vn);
		...
		if (trace_strip) {
			for (corner_cycler cc = this->get_corner_cycler(ck); !cc.cycle_complete(); cc.next()) {
				// check if the segment leaves through the corner opposite edge
				ck = cc.ci();
				unsigned int cp = this->prev(ck);
				unsigned int cn = this->next(ck);
			...
			if (!trace_strip) {
				std::cerr << "could not leave vertex " << this->vi_of_ci(ck) << std::endl;
				return;
			}
		}
...
*/
		...
		do {
			if (this->is_opposite_to_border(ck)) {
				std::cerr << "ray trace leaves convex hull" << std::endl;
				return;
			}
			ck = this->inv(ck);
			unsigned int vk = this->vi_of_ci(ck);
			if (vk == vj)
				return;

			const point_type& pk = this->p_of_vi(vk);
...
{
	return geometry_type::compute_circum_center(
			this->p_of_vi(this->vi_of_ci(ci)),
			this->p_of_vi(this->vi_of_ci(this->next(ci))),
			this->p_of_vi(this->vi_of_ci(this->prev(ci))));
}
...
/*	for (k=0; k<cis.size(); ++k) {
	...
	while (i < cis.size()) {
		...
		for (k=0; k<cis.size(); ++k) {
			if (cis[k] == -1)
				continue;
			std::cout << "(" << this->vi_of_ci(this->next(cis[k])) << "," << this->vi_of_ci(this->prev(cis[k])) << ") ";
		}
		std::cout << std::endl;
		unsigned int c0 = cis[j];
		unsigned int c1 = this->next(c0);
		unsigned int c2 = this->next(c1);
		unsigned int c3 = this->inv(c0);
		unsigned int c4 = this->next(c3);
		unsigned int v0 = this->vi_of_ci(c0);
		unsigned int v1 = this->vi_of_ci(c1);
		unsigned int v2 = this->vi_of_ci(c2);
		unsigned int v3 = this->vi_of_ci(c3);
		const point_type& p0 = this->p_of_vi(v0);
		const point_type& p1 = this->p_of_vi(v1);
		const point_type& p2 = this->p_of_vi(v2);
		const point_type& p3 = this->p_of_vi(v3);
...
{
	if (this->is_opposite_to_border(c0))
		return false;
	unsigned int c1 = this->next(c0);
	unsigned int c2 = this->next(c1);
	unsigned int c3 = this->inv(c0);
	const point_type& p0 = this->p_of_vi(this->vi_of_ci(c0));
	const point_type& p1 = this->p_of_vi(this->vi_of_ci(c1));
	const point_type& p2 = this->p_of_vi(this->vi_of_ci(c2));
	const point_type& p3 = this->p_of_vi(this->vi_of_ci(c3));
