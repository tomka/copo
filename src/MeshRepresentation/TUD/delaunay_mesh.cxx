...
template class delaunay_mesh<triangle_mesh<mesh_geometry<float> > >;
template class delaunay_mesh<>;
template class delaunay_mesh<triangle_mesh<mesh_geometry_reference<float> > >;
...
unsigned int delaunay_mesh<T>::find_nearest_neighbor(const point_type& p, unsigned int ci_start /* = 0 */) const
{
	...
	coord_type min_squared_dist = this->sqr_dist(p,this->p_of_vi(this->vi_of_ci(ci)));
	while (true) {
		unsigned int cj = ci;
		unsigned int ck = -1;
		...
		do {
			coord_type new_squared_dist = this->sqr_dist(p,this->p_of_vi(this->vi_of_ci(this->next(cj))));
			if (squared_dist == -1 || new_squared_dist < squared_dist) {
				squared_dist = new_squared_dist;
				ck = this->next(cj);
			}
			if (jump_boundary) {
				cj = this->next(this->prev_on_border(cj));
				jump_boundary = false;
			}
			else
				if (this->is_opposite_to_border(this->next(cj))) {
					cj = this->next(cj);
					jump_boundary = true;
				}
				else
					cj = this->next(this->inv(this->next(cj)));
...
{
	const point_type& p0 = this->p_of_vi(this->vi_of_ci(ci));
	const point_type& p1 = this->p_of_vi(this->vi_of_ci(this->next(ci)));
	const point_type& p2 = this->p_of_vi(this->vi_of_ci(this->prev(ci)));
	const point_type& p3 = this->p_of_vi(this->vi_of_ci(this->inv(ci)));
...
template <class T>
typename delaunay_mesh<T>::vertex_insertion_info delaunay_mesh<T>::insert_vertex(unsigned int vi, unsigned int ci_start)
{
	unsigned int n = this->get_nr_triangles();
	...
	if (vii.extends_border) {
		n = this->get_nr_triangles()-n;
		unsigned int ci = vii.ci_of_vertex;
		while (!this->is_opposite_to_border(this->prev(ci)))
			ci = this->prev(this->inv(this->prev(ci)));
		flip_edges_around_vertex_to_validate(ci, n);
	}
	else {
		flip_edges_around_vertex_to_validate(vii.ci_of_vertex, 3);
	}
	return vii;
}
...
template <class T>
void delaunay_mesh<T>::flip_edges_around_vertex_to_validate(unsigned int ci, unsigned int n)
{
	while (n > 0) {
		if (!this->is_opposite_to_border(ci) && !this->is_locally_delaunay(ci)) {
			this->flip_edge(ci);
			++n;
		}
		else {
			ci = this->next(this->inv(this->next(ci)));
			--n;
		}
	}
}
