...
namespace zx
{
	...
	public:
		delegate1_static(FnPtr pFn) : delegate1_base<A>(NULL, pFn) {;}
		delegate1_static(const delegate1_static& rhs) : delegate1_base<A>(rhs) {;}
		virtual delegate1_base<A>* Copy() { return new delegate1_static(*this); }

	protected:
		virtual void execute(A arg)
		{
			FnPtr pfn = (FnPtr)(this->GetFn());
			(*pfn)(arg);
		}
	};
	...
	public:
		delegate2_static(FnPtr pFn) : delegate2_base<A1,A2>(NULL, pFn) {;}
		delegate2_static(const delegate2_static& rhs) : delegate2_base<A1,A2>(rhs) {;}
	...
	public:
		delegate1(O* pObj, FnPtr pFn) : delegate1_base<A>(pObj, &pFn), m_pFn(pFn) {;}
		delegate1(const delegate1& rhs) : delegate1_base<A>(rhs) { m_pFn = rhs.m_pFn; }
		virtual delegate1_base<A>* Copy() { return new delegate1<O,T,A>(*this); }

	protected:
		virtual void execute(A arg)
		{
			O* pobj = (O*) (this->GetObject());
			(pobj->*m_pFn)(arg);
		}
	...
	public:
		delegate2(O* pObj, FnPtr pFn) : delegate2_base<A1, A2>(pObj, &pFn), m_pFn(pFn) {;}
		delegate2(const delegate2& rhs) : delegate2_base<A1, A2>(rhs) { m_pFn = rhs.m_pFn; }
